...
1. Lean source code lives in `src`. It is preprocessed using `scripts/preprocess.jq` {margin}[The script is copied from [`github/l-oksanen/lean-notes`][lauri-notes].] which turns the lean files into verso documents.
...
- Packaging verso with nix required adding `defaultFacets := #[LeanLib.staticFacet]` {margin}[Similar tricks are found in [leanPackages.batteries](https://github.com/NixOS/nixpkgs/blob/a799d3e3886da994fa307f817a6bc705ae538eeb/pkgs/development/lean-modules/batteries/default.nix#L28).] to all library targets as well as removing default targets (such as tests) which don't need to be built to use verso.
- We also need to patch verso to build `Verso.Output.Html.CssVars` and `Verso.Output.Html.KaTeX` which are not exported by default.
